...
public class MonitorViewModel : ViewModelBase
{
	...
	public void IncrementBrightnessNoCycle(int tickSize)
	{
		if (IsRangeChanging)
			return;

		var size = tickSize * GetRangeRate();
		var newBrightness = Brightness + (int)size;
		if (newBrightness > RangeHighest)
			newBrightness = RangeHighest;
	...
	public void DecrementBrightnessNoCycle(int tickSize)
	{
		if (IsRangeChanging)
			return;
		...
		var size = tickSize * GetRangeRate();
		var newBrightness = Brightness - (int)size;
		if (newBrightness < RangeLowest)
			newBrightness = RangeLowest;
		SetBrightness(newBrightness, false);
	}
