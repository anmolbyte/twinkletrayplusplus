...
public class SettingsCore : BindableBase
{
	...
	/// <summary>
	/// List of global hotkey bindings
	/// </summary>
	[DataMember]
	public ObservableCollection<HotkeyAction> Hotkeys
	{
		get => _hotkeys ??= new ObservableCollection<HotkeyAction>();
		set => SetProperty(ref _hotkeys, value);
	}
	private ObservableCollection<HotkeyAction> _hotkeys;
	...
	public SettingsCore()
	{
		KnownTypes = new[] { typeof(HotkeyAction), typeof(HotkeyActionType) };
	}
	...
	protected internal virtual async Task InitiateAsync()
	{
		...
		{
			if (hotkey.ActionType == HotkeyActionType.BrightnessSet)
			{
				if (hotkey.Value < 0) hotkey.Value = 0;
				if (hotkey.Value > 100) hotkey.Value = 100;
			}
			else
			{
				if (hotkey.Value == 0) hotkey.Value = 5;
			}
		}
	...
	private void Hotkeys_CollectionChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
	{
		OnPropertyChanged(nameof(Hotkeys));
		if (e.NewItems != null)
		{
			foreach (HotkeyAction item in e.NewItems)
			{
				item.PropertyChanged += Hotkey_PropertyChanged;
			}
		}
		if (e.OldItems != null)
		{
			foreach (HotkeyAction item in e.OldItems)
			{
				item.PropertyChanged -= Hotkey_PropertyChanged;
			}
		}
